...
def plot_tracks_v2(
  ...
  Args:
    ...
    point_size: int, the size of the plotted points, passed as the 's' parameter
      to matplotlib.
    rainbow_colors: bool, if True, the points will be plotted with a rainbow.
      Otherwise, the points will be plotted with random colors.
    marker: str, the marker to use for the points.  See matplotlib's scatter
  ...
  for i in range(rgb.shape[0]):
    ...
    if show_pred_occluded:
      vis_points = points * (1 - occluded)[...,np.newaxis]
      occ_points = points * occluded[...,np.newaxis]
      plt.scatter(vis_points[:, i, 0], vis_points[:, i, 1], s=point_size, c=colors, marker=marker)
      plt.scatter(occ_points[:, i, 0], occ_points[:, i, 1], facecolors='none', edgecolors=colors, linewidths=1, marker="o")
...
def estimate_homography(targ_pts, src_pts, mask=None):
  ...
  # for reference.
  print('total valid points:', np.sum(mask))
  if mask is None:
    mask = np.ones_like(targ_pts[..., 0])
  targ_x = targ_pts[..., 0]

  targ_y = targ_pts[..., 1]
  src_x = src_pts[..., 0]
  src_y = src_pts[..., 1]
  one = np.ones_like(targ_x)
  zero = np.zeros_like(targ_x)
  a1 = (
      np.stack(
          [
              src_x,
  ...
  if a.shape[0] <= 8:
    u, s, v = np.linalg.svd(a, full_matrices=True)  # pylint: disable=unused-variable
  else:
    u, s, v = np.linalg.svd(a, full_matrices=False)  # pylint: disable=unused-variable
...
def compute_inliers(
  ...
  if src_pts_homog is None:
    src_pts_homog = np.transpose(
        np.concatenate([src_pts, src_pts[:, 0:1] * 0 + 1], axis=-1)
    )
  tformed = np.transpose(np.matmul(homog, src_pts_homog))
  tformed = tformed[..., :-1] / (
      np.maximum(1e-12, np.abs(tformed[..., -1:]))
      * np.sign(tformed[..., -1:])
  )
  err = np.sum(np.square(targ_pts - tformed), axis=-1)
...
def ransac_homography(targ_pts, src_pts, vis, thresh=4.0, targ_inlier_frac=0.5):
  ...
  probs = vis / np.sum(vis)
  perm = np.array([np.random.choice(targ_pts.shape[0], 4, replace=False, p=probs) for _ in range(targ_pts.shape[0])])
  
  targ_pts_choice = np.take_along_axis(
      targ_pts[:, np.newaxis], perm[:, :, np.newaxis], axis=0
  )
  src_pts_choice = np.take_along_axis(
      src_pts[:, np.newaxis], perm[:, :, np.newaxis], axis=0
  )
  src_pts_homog = np.transpose(
      np.concatenate([src_pts, src_pts[:, 0:1] * 0 + 1], axis=-1)
  )
  ...
  def loop_body(arg):
    ...
    new_inliers = np.sum(np.array(new_inliers, np.int32))
    
    homog = np.where(new_inliers > inliers, homog, old_homog)

    return (it + 1, np.maximum(inliers, new_inliers), homog)
  ...
  def should_continue(arg):
    ...
    threshold = np.minimum(
        np.array(1 - (it + 1) / src_pts_choice.shape[0], np.float32),
        targ_inlier_frac * (0.99 ** np.array(it, np.float32)),
    )
    threshold = threshold * np.array(src_pts_choice.shape[0], np.float32)
    return np.array(inliers, np.float32) < threshold

  init = (0, 0, np.zeros([3, 3]))
  # Execute the loop
  while should_continue(init):
      init = loop_body(init)
  _, _, homog = init
  inliers = compute_inliers2(homog)
  final_homog = estimate_homography(
      targ_pts, src_pts, np.array(inliers, np.float32)
  )
...
def maybe_ransac_homography(*arg, thresh=4.0, targ_inlier_frac=0.5):
  ...
  vis = np.logical_and(np.logical_not(targ_occ), np.logical_not(src_occ))
  if np.sum(vis) > 4:
...
def compute_canonical_points(
  ...
  definite_outliers = np.logical_or(occ, err > outer_thresh)
  maybe_inliers = np.logical_and(np.logical_not(occ), err < inner_thresh)
  sum_inliers = np.sum(maybe_inliers, axis=0)
  sum_vis = np.sum(np.logical_not(occ), axis=0)
  frac_inliers = sum_inliers / np.maximum(1.0, sum_vis)
  canonical_invalid = frac_inliers < required_inlier_frac
  canonical_pts = np.einsum(
      'tnc,tn->nc', all_tformed, np.logical_not(definite_outliers)
  ) / np.maximum(
      1.0, np.sum(np.logical_not(definite_outliers), axis=0)[:, np.newaxis]
  )
  ...
  random_choice = np.floor(
      np.random.random([vis.shape[1]]) * np.sum(vis, axis=0)
  )
  ids = np.cumsum(vis, axis=0) * vis - 1 * occ
  idx = ids == random_choice[np.newaxis, :]
  idx = np.sum(
      idx * np.arange(vis.shape[0], dtype=np.int32)[:, np.newaxis], axis=0
  )[np.newaxis, :, np.newaxis]
  random_pts = np.take_along_axis(all_tformed, idx, axis=0)[0]
  canonical_pts = (
      canonical_invalid[:, np.newaxis] * random_pts
      + (1 - canonical_invalid[:, np.newaxis]) * canonical_pts
  )
...
def get_homographies_wrt_frame(
  ...
  res_homog[reference_frame] = np.eye(3)
  ...
  for j in range(num_refinement_passes):
    for fr in [reference_frame,] + after + before:
      ...
      homog = estimate_homography(
          ...
          # pts[reference_frame],
          pts[fr],
          np.array(np.logical_not(invalid), np.float32),
      )
...
def plot_tracks_tails(
  ...
  for i in range(rgb.shape[0]):
    ...
    for j in range(i - 1, -1, -1):
      ...
      if trail_fade:
        # changing opacity
        alpha = max(1 - 0.9 * ((i - j) / ((i + 1) * .99)), 0.1)
        colalpha2[:, -1] *= alpha
